...
use engine::parser::lexer::patterns::{token_start, verbatim_start, Extract};
    ...
    fn tokenize<'a>(job: &'a mut Job) -> Result<(), Traced<LexerError>> {
        ...
        match capture.tag {
            token_start::Tag::Comment => {
                try_traced!(Self::lex_comment(job));
            ...
            token_start::Tag::Block => {
                ...
                match job.patterns.verbatim_start.extract(job.cursor.tail()) {
                    Some(verbatim_start) => {
                        ...
                        try_traced!(Self::lex_verbatim_data(job, verbatim_start.tag));
                    ...
                    _ => {
                        ...
                        match job.patterns.block_line.extract(job.cursor.tail()) {
                            Some(block_line) => {
                                let block_line = try_traced!(block_line);
                            ...
                            _ => {
                                ...
                                try_traced!(state::Block::tokenize(job));
            ...
            token_start::Tag::Expression => {
                ...
                try_traced!(state::Expression::tokenize(job));
...
impl<'a> Data {
    fn lex_comment(job: &'a mut Job) -> Result<(), Traced<SyntaxError>> {
        match job.patterns.comment_end.find(job.cursor.tail()) {
            None => return traced_err!(SyntaxError::UnclosedComment { cursor: job.cursor.dump() }),
            Some(position) => job.cursor.move_by(position.1),
        }
        Ok(())
    }

    fn lex_verbatim_data(job: &'a mut Job,
                         tag: verbatim_start::Tag)
                         -> Result<(), Traced<SyntaxError>> {
        let capture = match job.patterns
                               .verbatim_end
                               .extract_iter(job.cursor.tail())
                               .find(|capture| capture.tag == tag) {
            Some(capture) => capture,
            _ => {
                return traced_err!(SyntaxError::UnexpectedEof {
    ...
    fn next_token_start_after_cursor(job: &'a mut Job) -> Option<token_start::ItemData> {
        ...
        job.token_start_iter.find(|x| x.position.0 >= position)
    }
}
...
#[cfg(test)]
mod test {
    use engine::parser::lexer::test::tokenize_err;
    use engine::parser::lexer::test::assert_tokenize;
    use engine::parser::token::Token;
    use engine::parser::lexer::{LexerError, SyntaxError};

    #[test]
    pub fn no_more_tokens() {
        assert_tokenize("only data no tokens",
                        vec![Token::Text("only data no tokens".to_string())])
    }

    #[test]
    pub fn comment() {
        assert_tokenize(" Hello \n {#- World -#} !",
                        vec![
    ...
    pub fn unclosed_comment() {
        ...
        let expect = LexerError::Syntax(SyntaxError::UnclosedComment { cursor: cursor_dump })
                         .to_string();

        let err = tokenize_err(" lost in space {#-").error().to_string();
        assert!(err.starts_with(&expect),
    ...
    pub fn block_verbatim() {
        assert_tokenize("{% verbatim %}<ul>{% for x in list %}<li>{{ x }}</li>{% endfor \
                         %}</ul>{% endverbatim %}",
                        vec![
                Token::Text("".to_string()),
    ...
    pub fn block_line() {
        ...
        let expect = LexerError::Syntax(SyntaxError::UnclosedBlock { cursor: cursor_dump })
                         .to_string();

        let err = tokenize_err("line1 {% line 99 %}\nline 2{%").error().to_string();
        assert!(err.starts_with(&expect),
                "error {:?} should start with {:?}",
                err,
                expect);
    ...
    pub fn unclosed_var() {
        ...
        let expect = LexerError::Syntax(SyntaxError::UnclosedVariable { cursor: cursor_dump })
                         .to_string();

        let err = tokenize_err(" or not \n to be !  {{").error().to_string();
        assert!(err.starts_with(&expect),
                "error {:?} should start with {:?}",
                err,
                expect);
