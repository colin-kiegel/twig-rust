...
impl<'a> Data {
    fn lex_comment(job: &'a mut Job) -> Result<(), Traced<SyntaxError>> {
        match job.patterns.comment_end.find(job.cursor.tail()) {
            None => return traced_err!(SyntaxError::UnclosedComment {
    ...
    fn lex_verbatim_data(job: &'a mut Job, tag: verbatim_start::Tag) -> Result<(), Traced<SyntaxError>> {
        let capture = match job.patterns.verbatim_end.extract_iter(job.cursor.tail())
                                                 .find(|capture| capture.tag == tag) {
            Some(capture) => capture,
            _ => return traced_err!(SyntaxError::UnexpectedEof {
...
mod test {
    ...
    pub fn unclosed_comment() {
        ...
        let expect = LexerError::Syntax(SyntaxError::UnclosedComment {
            cursor: cursor_dump
        }).to_string();
    ...
    pub fn block_line() {
        ...
        let expect = LexerError::Syntax(SyntaxError::UnclosedBlock {
            cursor: cursor_dump
        }).to_string();
    ...
    pub fn unclosed_var() {
        ...
        let expect = LexerError::Syntax(SyntaxError::UnclosedVariable {
            cursor: cursor_dump
        }).to_string();
